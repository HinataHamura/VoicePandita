...
export default function HistoryPage() {
  ...
  return (
    <div className="min-h-dvh bg-cream">
      <header className="sticky top-0 z-10 border-b border-forest/10 bg-cream/85 px-4 py-4 backdrop-blur-sm">
        <div className="mx-auto flex max-w-4xl items-center justify-between gap-3">
          ...
          {history.length > 0 && (
            <button onClick={clearAll} className="inline-flex items-center gap-2 rounded-lg border border-clay/20 bg-white/70 px-3 py-2 text-sm font-medium text-clay hover:bg-red-50">
              <Trash2 size={15} />
              Clear
            </button>
          )}
      ...
      <main className="mx-auto max-w-4xl px-4 py-6">
        {history.length === 0 ? (
          <div className="flex min-h-[50dvh] flex-col items-center justify-center text-center">
            <div className="mb-4 flex h-14 w-14 items-center justify-center rounded-2xl bg-forest/8 text-forest">
            ...
            <Link href="/learn" className="mt-5 rounded-full bg-saffron px-5 py-2.5 text-sm font-semibold text-white shadow-sm shadow-saffron/20">
              Ask a question
            </Link>
          </div>
        ) : (
            ...
            {history.map(item => (
              <article key={item.id} className="card p-5">
                <div className="mb-3 flex flex-wrap items-center gap-2 text-xs text-ink/45">
                  ...
                  <span className="rounded-full bg-forest/8 px-2 py-0.5 text-forest">{item.subject}</span>
                  <span className="rounded-full bg-saffron/10 px-2 py-0.5 text-saffron">{item.outputMode}</span>
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <p className="bangla rounded-lg bg-forest/7 px-3 py-2 text-sm leading-relaxed text-ink">{item.question}</p>
                  </div>
                  <div>
                  ...
                  {item.graphPath?.length ? (
                    <div className="rounded-lg border border-forest/10 bg-white/60 px-3 py-2 text-xs text-forest">
                      {item.graphPath.join(' -> ')}
                    </div>
                  ) : null}
                </div>
