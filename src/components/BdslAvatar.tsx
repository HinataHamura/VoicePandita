...
function CwasaSignAvatar({ token, playing }: { token: SignToken; playing: boolean }) {
  ...
  const playUrl = useCallback((url: string) => {
    ...
    setStatus('Loading SiGML...')

    const input = document.querySelector<HTMLInputElement | HTMLTextAreaElement>(
      '.CWASASiGMLURL.av0 input, .CWASASiGMLURL.av0 textarea, #CWASASiGMLURLav0 input, #CWASASiGMLURLav0 textarea',
    )
    if (input) {
      ...
      const playRoot = document.querySelector('.CWASAPlay.av0, #CWASAPlayav0') || hostRef.current
      const controls = playRoot
        ? Array.from(playRoot.querySelectorAll<HTMLElement>('button, input, a, [role="button"], *'))
        : []
      ...
      }) ?? controls.find((btn) => typeof btn.click === 'function' && btn.getAttribute('disabled') === null)
    ...
    const playSiGMLURL = window.CWASA?.playSiGMLURL
    if (typeof playSiGMLURL === 'function') {
      try {
        window.CWASA?.stopSiGML?.()
        playSiGMLURL(absoluteUrl)
        setStatus('Playing IsharaKotha SiGML')
        return
      } catch {
        try {
          playSiGMLURL(0, absoluteUrl)
          setStatus('Playing IsharaKotha SiGML')
          return
        } catch {
    ...
    setStatus('CWASA play control not ready')
  }, [])
  ...
  useEffect(() => {
    ...
    ensureCwasaLoaded()
      .then(() => {
        ...
        const initWhenReady = (retries = 20) => {
          if (cancelled || initializedRef.current) return
          if (!document.querySelector('.CWASAAvatar.av0')) {
            if (retries > 0) window.setTimeout(() => initWhenReady(retries - 1), 100)
            else setStatus('CWASA avatar container not ready')
            return
          }
          window.CWASA?.init?.()
  ...
  useEffect(() => { if (!playing) window.CWASA?.stopSiGML?.() }, [playing])
  ...
  return (
    <div ref={hostRef} style={{ position: 'relative', width: '100%', height: '340px', overflow: 'hidden' }} aria-label="CWASA BdSL signing avatar">
      ...
      <div style={{ position: 'absolute', left: '-10000px', top: '0', width: '420px', minHeight: '220px', opacity: 0, pointerEvents: 'none' }}>
        <span id="CWASAAvMenuav0" className="CWASAAvMenu av0" /><span id="CWASAAmbBoxav0" className="CWASAAmbBox av0" /><span id="CWASASpeedav0" className="CWASASpeed av0" />
        <div id="CWASASiGMLURLav0" className="CWASASiGMLURL av0" /><div id="CWASASiGMLTextav0" className="CWASASiGMLText av0" />
        <div id="CWASAPlayav0" className="CWASAPlay av0" /><span id="CWASAPlayExtraav0" className="CWASAPlayExtra av0" /><span id="CWASAFramesav0" className="CWASAFrames av0" />
        <div id="CWASAProgressav0" className="CWASAProgress av0" /><div id="CWASAStatusav0" className="CWASAStatus av0" />
